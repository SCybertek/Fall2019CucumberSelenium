...
Feature: Create a car
  ...
  Scenario: Add new car
    ...
    #in scenario : if we include more than one cra it is possible but if first car fails then the rest will not run
    # in scenario outline : when we create multiple cars even if one of the test is failing the rest will still continue running